...
class _WarnaPageState extends State<WarnaPage> {
      ...
      child: SafeArea(
        child: LayoutBuilder(builder: (context, box) {
          ...
          return Column(children: [
            ...
            Expanded(child: _tab == 0 ? _lesson(w, s) : MiniQuizPanel(
              questions: const [
  ...
  Widget _tabButton(String label, int value, double s) {
    ...
    return Material(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(18 * s),
        onTap: () { setState(() => _tab = value); audio.speak(label == 'WARNA' ? 'Belajar warna' : 'Mini kuis warna'); },
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        AnimatedContainer(
          ...
          height: w * .47, padding: EdgeInsets.all(22 * s),
          decoration: BoxDecoration(color: Color.lerp(const Color(0xFFFFF1D9), current.$2, .14), borderRadius: BorderRadius.circular(38 * s), border: Border.all(color: Colors.white, width: 3), boxShadow: const [BoxShadow(color: Color(0x330D405C), blurRadius: 14, offset: Offset(0, 7))]),
          child: Row(children: [
          ...
          child: GridView.builder(
            ...
            itemBuilder: (context, i) {
              ...
              return AnimatedScale(
                scale: i == selected ? 1.04 : 1.0, duration: const Duration(milliseconds: 220),
                child: Material(color: item.$2, borderRadius: BorderRadius.circular(24 * s), elevation: i == selected ? 9 : 4, child: InkWell(
                  onTap: () { setState(() => selected = i); audio.speak(item.$1); }, borderRadius: BorderRadius.circular(24 * s),
                  child: Container(alignment: Alignment.center, decoration: BoxDecoration(border: i == selected ? Border.all(color: Colors.white, width: 4) : null, borderRadius: BorderRadius.circular(24 * s)), child: FittedBox(child: Padding(padding: EdgeInsets.symmetric(horizontal: 12 * s), child: Text(item.$1, style: TextStyle(fontSize: 28 * s, color: dark ? Colors.white : Colors.black, fontWeight: FontWeight.w900))))),
                )),
              );
            },
          ),
  ...
  Widget _header(double w, double s) => SizedBox(height: w * .16, child: Stack(alignment: Alignment.center, children: [
    Positioned(left: 0, child: _round(w * .12, Icons.arrow_back_rounded, () { audio.speak('Kembali'); Navigator.of(context).maybePop(); })),
    Positioned(right: 0, child: _round(w * .12, Icons.volume_up_rounded, () => audio.speak('Belajar Warna. Mengenal Berbagai Warna'))),
  ...
  Widget _round(double size, IconData icon, VoidCallback onTap) => Material(color: icon == Icons.volume_up_rounded ? const Color(0xFF29C63E) : const Color(0xFFFFC42D), shape: const CircleBorder(), elevation: 6, child: InkWell(customBorder: const CircleBorder(), onTap: onTap, child: SizedBox(width: size, height: size, child: Icon(icon, color: Colors.white, size: size * .56))));
...
class _SplashPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final p = Paint()..color = color; final path = Path(); final c = Offset(size.width / 2, size.height / 2);
    for (var i = 0; i < 24; i++) { final a = i * math.pi / 12; final r = i.isEven ? size.width * .46 : size.width * .31; final x = c.dx + r * math.cos(a); final y = c.dy + r * math.sin(a); if (i == 0) path.moveTo(x, y); else path.lineTo(x, y); }
    path.close(); canvas.drawPath(path, p); canvas.drawCircle(c, size.width * .31, p);
  }
  @override bool shouldRepaint(covariant _SplashPainter old) => old.color != color;
}
